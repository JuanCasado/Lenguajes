...
class main{  
    public static void main(String[] args) throws Exception{
        EntradaParser parserEntrada = new EntradaParser(new CommonTokenStream(new EntradaLexer(args.length>0?new ANTLRInputStream(args[0]):new ANTLRInputStream(System.in))));
        parserEntrada.setBuildParseTree(true);
        HashContenedor hash1 = new EntradaVisitor().visit(parserEntrada.init());

        CsvParser parserCsv = new CsvParser(new CommonTokenStream(new CsvLexer(new ANTLRInputStream(new FileInputStream("/Users/mr.blissfulgrin/Documents/UAH_2018_2019/LENGUAJES/LAB/PEL2/Documentos/ficheros_a_procesar.csv")))));
